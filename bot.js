...
function parseBuyTransaction(tx, minThreshold) {
  try {
    ...
    return { volume: value, buyer, hash: tx.hash || '', timestamp: tx.utime || 0 };
  } catch (error) { console.error('Error parsing transaction:', error.message); return null; }
}

// ==================== УВЕДОМЛЕНИЯ ====================
async function sendBuyNotification(chatId, buyData, price) {
...
async function monitorTransactions() {
  ...
  for (const chatId of notificationChats) {
    ...
    for (const tx of transactions) {
      ...
      const buyData = parseBuyTransaction(tx, minThreshold);
      if (buyData) {
        await sendBuyNotification(chatId, buyData, price);
        if (buyData.timestamp > lastProcessedTimestamp) lastProcessedTimestamp = buyData.timestamp;
      }
    }
  }
}
